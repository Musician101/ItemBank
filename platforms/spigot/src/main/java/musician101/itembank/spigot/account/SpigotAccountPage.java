...
public class SpigotAccountPage extends AbstractAccountPage<InventoryCloseEvent, Inventory, ItemStack, Player, String, World> implements Listener
{
    ...
    protected void saveAccount(Inventory topInv, Inventory playerInv)
    {
        ...
        try
        {
            ...
            {
                plugin.getMySQLHandler().querySQL("CREATE TABLE IF NOT EXISTS ib_" + owner.toString().replace("-", "_") + "(World varchar(255), Page int, Slot int, Material varchar(255), Damage int, Amount int, ItemMeta varchar(300));");
                for (int slot = 0; slot < account.getSize(); slot++)
                {
                    plugin.getMySQLHandler().querySQL("DELETE FROM ib_" + owner + " WHERE World = \"" + world.getName() + "\" AND Page = " + page + " AND Slot = " + slot + ";");
                    ItemStack item = account.getItem(slot);
                    if (item != null)
                        plugin.getMySQLHandler().updateSQL("INSERT INTO ib_" + owner + "(World, Page, Slot, Material, Damage, Amount, ItemMeta) VALUES (\"" + world.getName() + "\", " + page + ", " + slot + ", \"" + item.getType().toString() + "\", " + item.getDurability() + ", " + item.getAmount() + ", \"" + serializeItem(item).replace("\"", "\\\"") + "\");");
                }
